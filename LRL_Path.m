function [distance] = LRL_Path(q1,q2)
%LRL_PATH Summary of this function goes here
%   q1 - stan poczatkowy
%   q2 - stan startowy
...
C1 = q1.LeftCircleCalc();
C2 = q2.LeftCircleCalc();

C1C2 = C2-C1;
if norm(C1C2) > 4*R
    ME = MException('PathError:TooFarForCCCPath', '|C1C2|>4R, sciezka LRL niewykonalna');
    throw(ME);
end
...
%laczny kat obrotu na C1 (obrot w lewo)
C1_alfa=mod(C1_angle2-C1_angle1+2*pi,2*pi);
%laczny kat obrotu na C3 (obrot w prawo)
C3_alfa=mod(C3_angle1-C3_angle2+2*pi,2*pi);
...
%C1->C3->C2
drawArc(C1,C1_angle1,C1_angle2,R); %luk na okregu C1
drawArc(C3,C3_angle2,C3_angle1,R); %luk na okregu C3
